...
const config = {
  SERVER_PORT: 1337,
  // SERVER_IP is now auto-detected for the link, or you can override it here.
  USERNAME: 'mono',
  PASSWORD: '1337',
};
...
const server = net.createServer((socket) => {
  socket.once('data', (data) => {
    ...
    socket.once('data', (authData) => {
      ...
      const passwd = authData.toString('utf8', plenIndex + 1, plenIndex + 1 + plen);
      ...
      socket.once('data', (requestData) => {
        ...
        if (addressType === 0x01) { // IPv4
          ...
          offset += 1;
          targetAddress = requestData.toString('utf8', offset, offset + addrLen);
          offset += addrLen;
...
server.listen(config.SERVER_PORT, () => {
  ...
  console.log(`https://t.me/socks?server=${detectedIP}&port=${config.SERVER_PORT}&user=${config.USERNAME}&pass=${config.PASSWORD}`);
  console.log(`(Note: If 'server' is 127.0.0.1, you cannot connect from another device. Use your LAN/Public IP)`);
});
